...
**内容装不下时可以用滚轮上下滚**（滚动条只在需要时出现）；日志那一条固定在底部，
不跟着滚，想看更多内容时点日志右上角的「收起」。
...
英语的对齐单位跟日语不一样。日语是摩拉计时，一个假名一个音；
英语是重音计时，一个音节能跨好几个音素（`through` = θ r uː 三个音素一个音节）。
所以英语这边用的单位是**元音核（音节）**，不是音素。

做法：文本 → 音素（CMU 发音词典，走 `cmudict` 包自带的数据）→ 按音节切 → IPA 音标。
音节边界按**最大音节首**切，合法性用词典统计出来。

```
Hello, it's me. I was wondering if after all these years you'd like to meet
→ hə·lˈoʊ ˈɪts mˈi ˈaɪ wˈɑz wˈʌn·dɚ·ɪŋ ˈɪf ˈæf·tɚ ˈɔl ðˈiz jˈɪɹz jˈud lˈaɪk tˈu mˈit
```

`wondering` 切三份 `wˈʌn·dɚ·ɪŋ`，`fire` 切两份 `fˈaɪ·ɚ`（元音核两个）。

中途踩了个坑：合法音节首是从词典里统计出来的，但词典混着方言/缩写词条，
带进了 `N D`（只出现 1 次）、`M B`（2 次）这种不该算音节首的簇，
结果 `somebody` 被切成 `sʌ·mbɑ·di`、`wondering` 切成 `wʌ·ndɚ·ɪŋ`。
加了频次门槛（真正的音节首簇都是高频的，`S T` 1900 次、`T R` 1199 次）后正常了。

**实测**（Adele《Hello》30 秒真实唱段；主流英文歌在网易云免登录只有试听，就用了一段）：

| | 音节数 | 音节→音符命中率（tol 0.12s） |
|---|---|---|
| 音节时间靠窗内均分 | 42 | 69.0% |
| **歌词作 context + 强制对齐** | 42 | **81.0%** |

均分那条低，限制在**时间来源**而不是音素分析。换成强制对齐后追平了日语摩拉的 83.9%。
对齐给出的时间也能对上官方歌词：`Hello` 落在 6.24s，官方 LRC 起点是 6.22s
（均分时它被算在 0.0s，差得远）。
...
拿无损 / Hi-Res **必须带上 cookie** —— 而且顺序很关键：程序会**先用带 cookie 的
app 接口**按你选的档位去问，只有它拿不到才退到免登录的明文接口。
（早先的顺序反了：明文接口排在最前，它给 320k 也算"成功"，于是会员也永远只拿到 320k。）
...
**扫码扫不动，或者不想扫码**：界面上「网易云」页的账号卡片里有一栏 **cookie 输入**，
把 `MUSIC_U=…` 那一段直接粘进去点「保存」就行（和扫码写的是同一个文件）。
点「载入当前」会把现在生效的那串填进输入框，方便改完再存。

> 二维码的有效期约 **5 分钟**（实测 301 秒，其间服务端一直返回 `等待扫码`）。
> 过期后程序会**自动换一张**，不用关掉重开；窗口里也能看到已经等了多久、
> 以及每次轮询返回的原始状态码。

> 登录成功的凭据是走**响应头 `Set-Cookie`** 回来的（正文里没有），
> 而且第一步取二维码时服务端会下发一个 `NMTID` 匿名设备标识 ——
> 必须整个扫码流程共用同一个 HTTP 会话、并且把响应头的 cookie 也收下来，
> 否则会出现「手机上明明扫成功了，电脑却说过期」。这两点已修。
...
这两条**不设也是开的**（2026-09-20 起就是出厂行为），要回到改动前设 `0`：

```
TS_HAND_GAP              1       人声在右手，且与伴奏至少差一个八度
TS_ACCOMP_BOOST          1       无人声段（纯伴奏）加强伴奏识别，含 other 电子音
```

**R1 —— 人声在右手，且与伴奏相差至少一个八度。**
原有的左右手分离只保证「左手 ≤59 / 右手 ≥60」，也就是两手只差 **1 个半音**，
谱面上仍然糊在一起。`TS_HAND_GAP=1` 会按时间窗把左手继续下移八度，
直到重叠时刻上 `右手最低音 − 左手最高音 ≥ 12` 半音；左手低到钢琴最低键（A0）
还不够的窗口，才反过来把人声那侧升八度。**只改音高**，不增删音、不改时值力度。

```
TS_HAND_GAP_SEMI         12      间隔下限（半音）
TS_HAND_GAP_FLOOR        21      左手最低音（21 = A0，钢琴最低键）
```

**R2 —— 无人声段加强伴奏识别，包括 other 里的电子音。**
前奏/间奏/尾奏原来是「再抽稀到 1.5 秒一个音 + 压低 15% 力度」，本意是不抢器乐主旋律，
代价是把本来就少的伴奏又削薄一半。更要命的是 `_filter_high_hallucination` 会删掉所有
「≥G5 且孤立」的音 —— 合成器主音、尖锐 pluck 正是这个形状。

`TS_ACCOMP_BOOST=1` 只改**无人声段**：不滤孤立高音、放宽长音抑制、抽稀更密，
并且把 `other` 轨**单独再识别一遍**只并进纯伴奏段。有人声段逐字节不变。
...
en_phoneme.py           英语 → 音素(ARPAbet) → 音节 → IPA 音标
...
- 音节对音符的命中率：日语摩拉整体 83.9%（前奏那段只有 10%），英语 81.0%（配强制对齐）。
  这个指标能定位到"哪几个字没弹出来"。
...
- 英语只在 30 秒试听段上验证过，没做过整曲 A/B；`en` 预设默认也不启用。
